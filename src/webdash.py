...
[html.Div([html.H1(f'CoSim Web Dashboard', style={'text-align': 'center'})], className='col-10'),
 ...
 html.Img(src='/assets/logoEC-Lab.png', style={"height": 100}))]), ]),
 html.Div(children=[
             ...
             html.Div(children=[
                 ...
                 html.Div(id='attr-selection', children=[
                     dcc.Dropdown(id='attr-name',
                                  multi=True, options=[{'label': s, 'value': s} for s in attrs_live]
                                  )]),
                 html.Div(id='attr-tuning', className='row', children=[
                     html.Div([dcc.Dropdown(id='attr-input', multi=False, options=[{'label': s, 'value': s} for s in
                                                                                   attrs_input.keys()])],
                              className='col-lg-2'),
                     html.Div([dcc.Input(id='input-value', value=None, type='number')], className='col-lg-1'),
                     html.Div([html.Button('Submit', id='submit', n_clicks=0)],className='col-lg-2'),
...
#
# def create_command_sender(port="5572"):
#     context = zmq.Context()
#     socket = context.socket(zmq.PUB)
#     socket.bind("tcp://127.0.0.1:%s" % port)
#     return socket

sender = zmq_sender(port=5572)
...
def send_value_attr(c, attr, value):
    ...
    if c > 0:
        if attr != 'empty':
            ...
            return html.P(f'Submitted {data}')
        else:
            return html.P('No tuning of parameters')
...
def update(data_names, n, attrs_options):
    ...
    if data_names:
        d = zmq_receiver()
        # [19500, {'Tair_ou': {'fmu_pyfmi_sim-0.rc_mo': 18.117763720994787}, 'PCooler': {'fmu_pyfmi_sim-0.rc_mo': 0.0},
        #         'PHeater': {'fmu_pyfmi_sim-0.rc_mo': 0.0}}]
        if bool(d):
            for attr in [x['label'] for x in attrs_options]:  # fmu_pyfmi_sim.rc_mo.Tair_ou
                sim_name = attr.split('.')[0]
                attr_name = attr.split('.')[2]  # Power
                entity = attr.split('.')[1]  # HeatPump_HP
                for sim_entity in d[1][attr_name].keys():  # fmu_pyfmi_sim-0.rc_mo
                    live_cache[attr].append(d[1][attr_name][sim_entity])

            live_cache['t'].append(d[0])  # start_date + datetime.timedelta(seconds=d[0]))
            for data_name in data_names:
                x_date =[start_time+datetime.timedelta(seconds=x) for x in live_cache['t']]
                ...
                graphs.append(html.Div(dcc.Graph(
                    id=data_name,
                    animate=False,
                    figure={'data': [data], 'layout': go.Layout(xaxis=dict(range=[min(x_date),
                                                                                  max(x_date)],type='date'),
