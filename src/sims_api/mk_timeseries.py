...
from src.definitions import MODELS_ROOT
...
class TimeSeriesSim(mosaik_api_v3.Simulator):
    ...
    def init(self, sid, datafile, start_date, stop_date=None, stepsize=None, time_resolution=1., sim_meta=None,
        ...
        self.step_ts = self.stepsize*self.time_resolution
        ...
        if self.step_ts != None:
            print("Resampling of the timeseries..")
            if self.step_ts > stepsize_from_df:
                # Resampling wrt stepsize
                self.df = self.df.resample(pd.Timedelta(seconds=self.step_ts)).mean()
            elif self.step_ts < stepsize_from_df:
                self.df = self.df.resample(pd.Timedelta(seconds=self.step_ts)).fillna("nearest").interpolate('akima')
        else:
            self.step_ts = stepsize_from_df
    ...
    def step(self, time, max_advance, inputs=None):
        expected_date = self.start_date + pd.Timedelta(seconds=time*self.time_resolution)
        self.timesteps = round(time / self.stepsize)
        # Chacke date
        try:
...
if __name__ == '__main__':
    mosaik_api_v3.start_simulation(TimeSeriesSim())
