...
from src.utilities.fmu_checker import fmu_checker as fmuchk
from src.definitions import MODELS_ROOT, mk_sims_api_config
...
class FMU_Adapter(mosaik_api_v3.Simulator):
    ...
    def create(self, num, model, extra_call=False, fmu_name=None, instance_name=None,
        ...
        for i in range(len(instance_name)):
            ...
            self.entities[eid] = load_fmu(fmu_path, log_level=self.fmu_log)  # til 7
            ...
            if fmiVersion == '2.0':
                self.entities[eid].setup_experiment(start_time=self.start_time * self.time_resolution,
                                                    stop_time=self.stop_time * self.time_resolution)
                ...
                if start_in_vrs:
                    self.entities[eid].set(list(start_in_vrs.keys()), list(start_in_vrs.values()))
                self.entities[eid].initialize(start_time=self.start_time * self.time_resolution,
                                              stop_time=self.stop_time * self.time_resolution)
    ...
    def solver_call(self, solver, num=1):
        if solver == 'matlab':
            log.info(f'Starting solver {solver} #{num}.')
            self.command = Popen([mk_sims_api_config['solvers']['matlab'] + "/fmu-matlab-setup.cmd", f'{num}'], stdin=PIPE,
                                 stdout=PIPE, stderr=PIPE)

            log.info(mk_sims_api_config['solvers']['matlab'] + "/fmu-matlab-setup.cmd")
            while self.command.stdout.readline().decode('utf-8').split('.')[0] != f'Launched MATLAB #{num}':
                pass
            log.info(f'Launched MATLAB #{num}')
    ...
    def step(self, time, inputs, max_advance):  # time it's current simualtion time
        ...
        if time != 0:  # step 0 escluso per allineamento
            for eid, fmu in self.entities.items():
                fmu.do_step(current_t=self.fmu_time * self.time_resolution,
                            step_size=(target_time - self.fmu_time) * self.time_resolution,
                            new_step=True)
...
if __name__ == '__main__':
    desc = __doc__.strip()
    mosaik_api_v3.start_simulation(FMU_Adapter(), desc)
