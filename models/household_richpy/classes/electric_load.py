...
class ElectricLoad(object):
    ...
    def __init__(self, occ_profile, total_nb_occ, q_direct, q_diffuse,
                 ...
                 save_app_light=False, stop_time=365, stepped=True):
        """
        ...
        # Stepped parameters
        self.sim_time = 0
        self.stop_time = stop_time  # stop time in days
        self.stepped = stepped
        if self.stepped:
            self.config_demand(is_sfh=is_sfh,
                      path_app=path_app, path_light=path_light,
                      randomize_appliances=randomize_appliances,
                      prev_heat_dev=prev_heat_dev, light_config=light_config,
                      timestep=timestep)
            ...
            for i in range(int(self.stop_time)): # i = day
                el_p_curve, light_p_curve, app_p_curve = self.step(i, timestepsDay, irradiance, required_timestamp,
                                                                   given_timestamp,
            ...
            self.calc_stoch_el_profile(q_direct=q_direct, q_diffuse=q_diffuse,
                                       ...
                                       save_app_light=save_app_light, stop_time=stop_time)

    def config_demand(self, is_sfh=True,
    ...
    def step(self, i, timestepsDay, irradiance, required_timestamp, given_timestamp,
             initial_day=1):
        ...
        return el_p_curve, light_p_curve, app_p_curve
    ...
    def calc_stoch_el_profile(self, q_direct, q_diffuse, is_sfh=True,
                              ...
                              save_app_light=False,stop_time =365):
        """
